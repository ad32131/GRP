...
//n.toFixed(decimals).toString().replace(/(\d)(?=(\d{3})+(?!\d))/g,"$1,");
    }
...
//at1
        ...
        parseBet: function (betString) {
            ...
            if (bet < 1000)
                return new Error('최소 배팅 금액은 1000원 입니다.');

            if (bet * 100 > AppConstants.Engine.MAX_BET)
                return new Error('최대 배팅금액 ' + formatSatoshis(AppConstants.Engine.MAX_BET) + '원을 초과 배팅 할 수 없습니다.');
	    if ( (bet % 100) != 0 )
		return new Error('100원 이하단위의 금액을 배팅 할 수 없습니다.');
