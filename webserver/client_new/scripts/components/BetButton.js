            ...
            if(notPlayingOrBetting) {
                ...
                if(isBetting) {
                    ...
                    if (this.props.engine.gameState !== 'STARTING') {
                        btnContent.push(D.span({ key: 'bc-0'}, smallButton? '' : '배팅중...'), D.a({ className: 'cancel', key: 'bc-1' }, ' (취소)'));
                        onClickFun = this.props.cancelBet;
                        btnClasses += ' cancel';
                    } else {
                        btnContent.push(D.span({ key: 'bc-0'}, '배팅중...'));
                    }

                    //Initial disable
                } else if(this.state.initialDisable) {
                	var btnText =   (canUserBet.message === 'Not enough bits')? (smallButton? 'Bet' : 'Bet too big') : (smallButton? 'Bet' : '배팅');
                    ...
                    if(invalidBet) {
                    	...
                    	var btnText =   (canUserBet.message === '잔고가 부족합니다')? (smallButton? 'Bet' : '배팅 금액이 너무 큽니다') : (smallButton? 'Bet' : '배팅');
                    ...
                    //Placing bet
                        ...
                        btnContent.push(D.span({ key: 'bc-4' }, smallButton? 'Bet' : '배팅'));
                        btnClasses += ' disable unselect';

                    //Able to bet
                    } else {
                        btnContent.push(D.span({ key: 'bc-5' }, smallButton? 'Bet' : '배팅'));
                        btnClasses += ' ';
            ...
            //The user is playing
                ...
                btnContent.push(
                    D.div({ className: 'btn-content', key: 'bc-6' },
                        D.span({ className: 'cashout-cont' }, '스탑'),
