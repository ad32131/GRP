...
function Chat(io) {
    ...
    function onConnection(socket) {  //socket.user is attached on the login middleware
        ...
        socket.on('disconnect', function() {
            --self.clientCount;
            console.log('접속 해지, left: ', self.clientCount);
        ...
        ++self.clientCount;
        console.log('고객 접속: ', self.clientCount, ' - ', socket.user ? socket.user.username : '~guest~','접속지IP' ,socket.handshake.address); //TODO: Add ip address
    }
...
Chat.prototype.onSay = function(socket, message, isBot, customChannelName) {
    if (!socket.user)
        return sendError(socket, '[say] 로그인을 하세요');
    ...
    if(customChannelName)
        if(isChannelNameInvalid(customChannelName))
            return sendError(socket, '[say] 채널 이름이 유효하지 않습니다');
        else
