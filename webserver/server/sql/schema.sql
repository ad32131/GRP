...
CREATE TYPE read_type AS ENUM ('not read', 'read');
...
content text NOT NULL,
owner text NOT NULL,
on_read read_type DEFAULT 'not read' NOT NULL,
reply text NOT NULL,
date timestamp with time zone DEFAULT now() NOT NULL
);

CREATE TYPE ch_status AS ENUM ('wait', 'accept', 'refuse','complete');

create table charge(
username text NOT NULL,
amount bigint NOT NULL,
status ch_status DEFAULT 'wait' NOT NULL,
date timestamp with time zone DEFAULT now() NOT NULL
);
...
CREATE TYPE ch_type AS ENUM ('charge', 'exchange');

create table chex_log(
username text NOT NULL,
amount bigint NOT NULL,
date timestamp with time zone DEFAULT now() NOT NULL,
type ch_type DEFAULT 'charge' NOT NULL
);

-- Users

CREATE TYPE UserClassEnum AS ENUM ('user', 'moderator', 'admin');
CREATE TYPE UserStat AS ENUM ('wait', 'ack', 'reject');
...
CREATE TABLE users (
    ...
    code text NOT NULL,
    all_charge_money bigint DEFAULT 0 NOT NULL,
    all_exch_money bigint DEFAULT 0 NOT NULL,
    userack UserStat DEFAULT 'wait' NOT NULL,
    exch_password text NOT NULL
);
...
GRANT ALL PRIVILEGES ON DATABASE bustabitdb to bustabit;
GRANT ALL PRIVILEGES ON DATABASE bustabitdb to moneypot;
GRANT ALL PRIVILEGES ON TABLE chex_log to bustabit;
GRANT ALL PRIVILEGES ON TABLE chex_log to moneypot;
GRANT ALL PRIVILEGES ON TABLE charge to bustabit;
GRANT ALL PRIVILEGES ON TABLE charge to moneypot;
GRANT ALL PRIVILEGES ON TABLE exchange_req to bustabit;
GRANT ALL PRIVILEGES ON TABLE exchange_req to moneypot;
GRANT ALL PRIVILEGES ON TABLE notice_board to bustabit;
GRANT ALL PRIVILEGES ON TABLE notice_board to moneypot;
GRANT ALL PRIVILEGES ON TABLE inq_board to bustabit;
GRANT ALL PRIVILEGES ON TABLE inq_board to moneypot;
