...
var restartTime = 2000; // How long from  game_starting -> game_started
...
//at2
	   console.log(config.CRASH_AT,'최대 상한선');

            if (config.CRASH_AT) {
  	    if( self.crashPoint > config.CRASH_AT){
                assert(!config.PRODUCTION);
                self.crashPoint = parseInt(config.CRASH_AT);
		console.log(self.crashPoint,'크래시 포인트');
            }
	}	
		console.log(self.crashPoint,'크래시 포인트');
	   
            self.hash = info.hash;
            console.log(self.hash,'현재 해시');
            self.gameId++;
    ...
    function runTick() {
	    ...
	    console.log(at,self.crashPoint,config.CRASH_AT,'현재:크래시:상한선');

	fs = require('fs');
	data = fs.readFileSync('./../bustabit-gameserver/server/crash_set.txt', 'utf8');
// wait for the result, then use it


	fs.writeFile('./../bustabit-gameserver/server/crash_rel.txt', at, function(err) {
	  if(err) throw err;
	});

	if(data != '1500')
	{
		self.crashPoint = data;


	fs.writeFile('./../bustabit-gameserver/server/crash_set.txt', '1500', function(err) {
	  if(err) throw err;
	});
